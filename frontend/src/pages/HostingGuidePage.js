...
export default function HostingGuidePage() {
  ...
  return (
    <div className="min-h-screen bg-gray-50" data-testid="hosting-guide-page">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <Button variant="ghost" onClick={() => navigate(-1)} className="mb-6 text-gray-500 hover:text-gray-900" data-testid="back-button">
          <ArrowLeft className="w-4 h-4 mr-2" /> Back
        </Button>

        <div className="mb-10">
          <h1 className="font-heading text-3xl sm:text-4xl font-black text-gray-900">Hostinger VPS Deployment Guide</h1>
          <p className="text-gray-500 mt-2">Step-by-step instructions to deploy Blogs 4 Blocks on a Hostinger VPS.</p>
        </div>

        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-8">
          <p className="text-sm text-amber-800 font-medium">Prerequisites</p>
          <ul className="text-sm text-amber-700 mt-1 space-y-1 list-disc ml-4">
        ...
        <div className="space-y-6">
          {steps.map((step, i) => (
            <div key={i} className="bg-white rounded-2xl border border-gray-100 overflow-hidden" data-testid={`deployment-step-${i + 1}`}>
              <div className="flex items-center gap-3 px-6 py-4 border-b border-gray-50">
                ...
                <div className="flex items-center gap-2 text-gray-900 font-heading font-bold">{step.icon} {step.title}</div>
              </div>
              <div className="p-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 mt-8">
          <h3 className="font-heading font-bold text-lg text-gray-900 mb-3">After Deployment</h3>
          <ol className="space-y-2 text-sm text-gray-600 list-decimal ml-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 mt-4">
          <h3 className="font-heading font-bold text-lg text-gray-900 mb-3">Troubleshooting</h3>
